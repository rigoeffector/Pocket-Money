...
# Script to fix negative remaining balance on LOCAL database
# Reads database configuration from application-dev.properties
# Usage: ./fix_negative_remaining_balance_local.sh

set -e  # Exit on error

echo "=========================================="
echo "Fix Negative Remaining Balance - LOCAL"
echo "=========================================="
echo ""
...
# Run the migration
PGPASSWORD="$DB_PASSWORD" psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -f fix_negative_remaining_balance.sql
